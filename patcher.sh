...
if [ ! -d "$TOOLSDIR/update_payload_extractor" ]; then
    ...
    cd "$TOOLSDIR/update_payload_extractor" && git pull
fi
...
if [ "$VERBOSE" = "n" ]; then
    sh "$LOCALDIR/extractor.sh" "$BASE_FIRMWARE" "$TMPDIR/source" > /dev/null 2>&1
else
    sh "$LOCALDIR/extractor.sh" "$BASE_FIRMWARE" "$TMPDIR/source"
fi

for OTA_FILE in "$@"; do
    OTA_NO=$((OTA_NO+1))
    echo "Patching OTA Number $OTA_NO"
    if ! (7z l -ba "$OTA_FILE" | grep -q "payload.bin"); then
